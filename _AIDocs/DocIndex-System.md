...
### lib/ — 共用模組（atom funnel 核心，2026-05-04 S1）
...
- atom_io.py — 全系統 atom 寫入唯一 funnel（**2026-05-04 S1.3 新增**）。`write_atom(*, source: REQUIRED, ...)` 對拍 server.js:1065 byte-identical（含 build+validate+atomic write+index+audit log）；`write_raw` escape hatch（failures/episodic 子族不符 build_atom_content 規範）；`update_atom_field` 單欄位 in-place（Confirmations/ReadHits 計數）；`write_index_full` 整檔重組（4 欄 schema 用）；`write_index` row-by-row append（3 欄）。`VALID_SOURCES` frozenset 9 source（mcp / hook:* 4 / tool:* 6）；ULID-ish `audit_id` + `_meta/atom_io_audit.jsonl` 反向證明日誌
- atom_io_cli.py — thin CLI bridge（**2026-05-04 S1.3 新增**）。stdin JSON action+kwargs → write_*/update_atom_field → stdout `WriteResult.to_dict()`。供 server.js MCP toolAtomWrite/Promote spawn `python -m lib.atom_io_cli`（S3.2 切 spawn 後省 600+ 行 byte-equivalence 重複實作）
