...
- atom-injection-summary.py — REG-005 atom 注入觀察期自動判定（Session 1 skeleton, 2026-04-29）。讀 `~/.claude/Logs/atom-injection-observation-*.log` 與 `memory/wisdom/reflection_metrics.json` 的 `reg005_observation` 子鍵，依「事件驅動四重標準」（A 注入 ≥150 + B session ≥6 + 2 ≤ wall_clock ≤ 7 天）與自動止血條件（hot 命中 / cold 主動讀 / 命中錯失 / 零覆蓋，皆有最低樣本門檻）輸出 KEEP / ROLLBACK / GRAY / INCOMPLETE / NOT_STARTED 判定。配對 hook：`hooks/wg_atom_observation.py`（flag-gated 採樣，flag = `memory/_staging/reg-005-observation-start.flag`）。Session 1 階段未啟動，注入端 instrumentation 留待 Session 2 補（C-layer hot/cold 分級）。`--report` 文字 / `--json` 結構化輸出。設計：`memory/_staging/reg-005-atom-injection-refactor.md`
