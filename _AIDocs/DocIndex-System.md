...
- audit-reconcile.py — **2026-05-04 S3.4 新增**：動態對拍 — 列近期 mtime atom × `_meta/atom_io_audit.jsonl` entries，mtime 沒對應 audit ts → 列為 suspect bypass。`--since 30s/2h/1d` 控制窗口，`--json` 結構化輸出。配 PreToolUse 強制門禁構成「靜態（check-bypass）+ 動態（audit-reconcile）」雙軌反向證明
